...
export function initApp() {
  ...
  // =========================================================
  // iOS (Safari/WebKit) AUDIO UNLOCK — synchronous, no await
  // =========================================================
  let iosUnlocked = false;

  function unlockIOSAudioSync() {
    if (iosUnlocked) return;

    // Kick off enable (creates ctx/master synchronously inside audio.enable)
    // We intentionally do NOT await on iOS.
    try {
      audio.enable();
    } catch (_) {}
    ...
    // Resume without awaiting (await can lose "gesture" on iOS)
    try {
      if (ctx.state !== "running") ctx.resume();
    } catch (_) {}

    // Critical: play a tiny blip to fully unlock output
    try {
      const o = ctx.createOscillator();
      const g = ctx.createGain();
      g.gain.value = 0.001; // tiny but non-zero (more reliable than 0.0001 on iOS)
  ...
  // Most reliable for iOS: touchstart
  document.addEventListener("touchstart", unlockIOSAudioSync, {
    passive: true,
    once: true,
  });
  // Also helpful on desktop / other devices
  document.addEventListener("pointerdown", unlockIOSAudioSync, { once: true });
  ...
  function setTab(which) {
    ...
    if (which === "play") {
      ...
      ui.panePlay.style.display = "block";
    }
    if (which === "capture") {
      ui.tabCapture.classList.add("active");
      ui.paneCapture.style.display = "block";
    }
    if (which === "tune") {
      ui.tabTune.classList.add("active");
      ui.paneTune.style.display = "block";
    }
  }
  ...
  function updateDebug() {
    ...
    const { et, realityC, realityHz, detuneC, outHz } = getOutputHz(
      ui,
      importedCurve,
      detuneMap,
      selectedMidi
  ...
  function attachKeyHandlers(div, midi) {
    const down = (e) => {
      ...
      // Ensure audio is unlocked on iOS before anything else
      unlockIOSAudioSync();
      ...
      // Press will no-op if ctx not created; unlockIOSAudioSync kicked it
      audio.press(midi);
      updateKeyHighlights();
    };
    ...
    try {
      ...
      if (!Array.isArray(curve) || curve.length !== 88) {
        ui.importStatus.textContent =
          "Import failed: curveCents88 must be an array of 88 numbers.";
        return;
      }
      const cleaned = curve.map((x) =>
        typeof x === "number" ? x : parseFloat(x)
      );
      if (cleaned.some((x) => Number.isNaN(x))) {
        ui.importStatus.textContent =
          "Import failed: curveCents88 contains non-numeric values.";
        return;
      }
